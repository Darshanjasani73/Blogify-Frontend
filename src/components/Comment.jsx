...
const Comment = ({ comment, onLike, onEdit, onDelete }) => {
  ...
  useEffect(() => {
    const getUser = async () => {
      try {
        const res = await fetch(`https://blogify-backend-1-0stb.onrender.com/api/user/${comment.userId}`);
  ...
  const handleSave = async()=>{
    try {
      const res = await fetch(`https://blogify-backend-1-0stb.onrender.com/api/comment/editComment/${comment._id}`, {
        method: 'PUT',
        headers: {
