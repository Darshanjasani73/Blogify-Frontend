...
const CreatePost = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFile) {
      ...
      setImageUploadError(null); // Clear previous errors when a new file is selected
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref)
          ...
          .catch((error) => {
            ...
            setImageUploadProgress(null);
            console.log("Get download URL error:", error.message);
          });
      }
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`https://blogify-backend-1-0stb.onrender.com/api/post/create`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        ...
        console.log("Publish error:", data.message || "Failed to publish the post");
        return;
      }
